...
public class AdicionarArtistasMusicaView extends JFrame {
	...
	public AdicionarArtistasMusicaView() {
		try {
			sistema = new Sistema();
		...
		addWindowListener(new WindowAdapter() {
			public void windowOpened(WindowEvent arg0) {
				...
				if(table.getRowCount()==0){
					dispose();
					JOptionPane.showMessageDialog(null, "Sem artistas dispon�veis para adicionar!");
				}
			}
		});
		...
		JLabel lblMusicasParaAdicionae = new JLabel("ARTISTAS PARA ADICIONAR");
		lblMusicasParaAdicionae.setFont(new Font("Segoe UI Symbol", Font.BOLD, 15));
		lblMusicasParaAdicionae.setBounds(99, 109, 389, 20);
		...
		sair.setBounds(266, 343, 92, 23);
		contentPane.add(sair);
		
		JScrollPane scrollPane = new JScrollPane();
		scrollPane.setBounds(10, 140, 389, 192);
		...
		textPCodigo.setBounds(89, 39, 305, 20);
		...
		textPNome.setBounds(89, 70, 305, 20);
		...
		separator.setBounds(15, 101, 384, 2);
		...
		selecionar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if (table.getSelectedRow()!=-1){
					MusicaView.tfCodigoMusica.setText(String.valueOf(table.getValueAt(table.getSelectedRow(), 0)));
					try {
						MusicaView.AdicionarArtista();
					} catch (InsertException | SelectException | NaoCadastradoException e) {
						JOptionPane.showMessageDialog(null, e.getMessage(), "ERRO", JOptionPane.ERROR_MESSAGE);
					}
					MusicaView.tfCodigoMusica.setText("");
					dispose();
				}else JOptionPane.showMessageDialog(null, "Nenhuma m�sica selecionada!");
			}
		});
		selecionar.setBackground(SystemColor.window);
		selecionar.setBounds(35, 347, 99, 23);
		...
		lblNewLabel.setBounds(0, -34, 416, 477);
	...
	public static void atualizarTabela() {
		try {
			artistas = sistema.listarArtistasMusicaAdiconar(Integer.parseInt(MusicaView.tfCodigo.getText()));
			DefaultTableModel model = (DefaultTableModel) table.getModel();
			model.setNumRows(0);
