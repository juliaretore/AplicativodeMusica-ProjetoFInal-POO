...
public class AdicionarMusicasArtistaView extends JFrame {
	...
	public AdicionarMusicasArtistaView() {
		try {
			sistema = new Sistema();
		...
		lblBusca.setBounds(155, 0, 69, 20);
		...
		lblMusicasParaAdicionae.setBounds(99, 103, 345, 20);
		...
		sair.setBounds(271, 322, 92, 23);
		contentPane.add(sair);
		
		JScrollPane scrollPane = new JScrollPane();
		scrollPane.setBounds(10, 122, 379, 189);
		...
		label_6.setBounds(10, 28, 69, 20);
		contentPane.add(label_6);
		label_6.setHorizontalAlignment(SwingConstants.RIGHT);
		
		textPCodigo = new JTextField();
		textPCodigo.setBounds(89, 31, 305, 20);
		...
		textPNome.setBounds(89, 62, 305, 20);
		...
		label_7.setBounds(20, 59, 59, 20);
		contentPane.add(label_7);
		label_7.setHorizontalAlignment(SwingConstants.RIGHT);
		
		JSeparator separator = new JSeparator();
		separator.setForeground(Color.LIGHT_GRAY);
		separator.setBounds(5, 93, 384, 2);
		...
		selecionar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if (table.getSelectedRow()!=-1){
					ArtistaView.tfCodigoMusica.setText(String.valueOf(table.getValueAt(table.getSelectedRow(), 0)));
					try {
						ArtistaView.AdicionarMusica();
					} catch (InsertException | SelectException | NaoCadastradoException e) {
						JOptionPane.showMessageDialog(null, e.getMessage());
					}
					ArtistaView.tfCodigoMusica.setText("");
					dispose();
				}else JOptionPane.showMessageDialog(null, "Nenhuma m�sica selecionada!");
			}
		});
		selecionar.setBackground(SystemColor.window);
		selecionar.setBounds(20, 322, 99, 23);
		...
		btnTocarPrevia.setBounds(155, 322, 92, 23);
	...
	public static void atualizarTabela() {
		try {
			musicas = sistema.listarMusicasArtistaAdicionar(Integer.parseInt(ArtistaView.tfCodigo.getText()));
			DefaultTableModel model = (DefaultTableModel) table.getModel();
			model.setNumRows(0);
